...
const mattjamNameSchemeAsk = "ask"
const mattjamNameSchemeWords = "words"
const mattjamNameSchemeUUID = "uuid"
const mattjamNameSchemeMattermost = "mattermost"
...
func (p *Plugin) OnActivate() error {
	...
	command, err := p.createMattJamCommand()
	if err != nil {
		return err
	}
	...
	mattjamBot := &model.Bot{
		Username:    "mattjam",
		DisplayName: "MattJam",
		Description: "A bot account created by the mattjam plugin",
	}
	options := []plugin.EnsureBotOption{
		plugin.ProfileImagePath("assets/icon.png"),
	}

	botID, ensureBotError := p.Helpers.EnsureBot(mattjamBot, options...)
	if ensureBotError != nil {
		return errors.Wrap(ensureBotError, "failed to ensure mattjam bot user.")
	}
...
// Claims extents cristalhq/jwt standard claims to add mattjam-web-token specific fields
type Claims struct {
	jwt.StandardClaims
...
func (p *Plugin) updateJwtUserInfo(jwtToken string, user *model.User) (string, error) {
	secret := p.getConfiguration().MattJamAppSecret
...
func (p *Plugin) startMeeting(user *model.User, channel *model.Channel, meetingID string, meetingTopic string, personal bool, rootID string) (string, error) {
	...
	if meetingID == "" {
		meetingID = encodeMattJamMeetingID(meetingTopic)
		if meetingID != "" {
			meetingID += "-"
		}
	...
	defaultMeetingTopic := p.b.LocalizeDefaultMessage(l, &i18n.Message{
		ID:    "mattjam.start_meeting.default_meeting_topic",
		Other: "MattJam Meeting",
	})
			...
			if channel.Type == model.CHANNEL_DIRECT || channel.Type == model.CHANNEL_GROUP {
				...
				meetingTopic = p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
					DefaultMessage: &i18n.Message{
						ID:    "mattjam.start_meeting.personal_meeting_topic",
				...
				meetingTopic = p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
					DefaultMessage: &i18n.Message{
						ID:    "mattjam.start_meeting.channel_meeting_topic",
	...
	mattjamURL := strings.TrimSpace(p.getConfiguration().GetMattJamURL())
	mattjamURL = strings.TrimRight(mattjamURL, "/")
	meetingURL := mattjamURL + "/" + meetingID
	meetingLink := meetingURL

	var meetingLinkValidUntil = time.Time{}
	JWTMeeting := p.getConfiguration().MattJamJWT
	var jwtToken string

	if JWTMeeting {
		// Error check is done in configuration.IsValid()
		jURL, _ := url.Parse(p.getConfiguration().GetMattJamURL())

		meetingLinkValidUntil = time.Now().Add(time.Duration(p.getConfiguration().MattJamLinkValidTime) * time.Minute)

		claims := Claims{}
		claims.Issuer = p.getConfiguration().MattJamAppID
		claims.Audience = []string{p.getConfiguration().MattJamAppID}
		claims.ExpiresAt = jwt.NewNumericDate(meetingLinkValidUntil)
		claims.Subject = jURL.Hostname()
		claims.Room = meetingID

		var err2 error
		jwtToken, err2 = signClaims(p.getConfiguration().MattJamAppSecret, &claims)
		if err2 != nil {
			return "", err2
		}
	...
	if JWTMeeting {
		meetingUntil = p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "mattjam.start_meeting.meeting_link_valid_until",
	...
	meetingTypeString := p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
		DefaultMessage: &i18n.Message{
			ID:    "mattjam.start_meeting.meeting_id",
			Other: "Meeting ID",
		},
	})
	if meetingPersonal {
		meetingTypeString = p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "mattjam.start_meeting.personal_meeting_id",
	...
	slackAttachment := model.SlackAttachment{
		Fallback: p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID: "mattjam.start_meeting.fallback_text",
...
[Join Meeting]({{.MeetingURL}})`,
		...
		Text: p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID: "mattjam.start_meeting.slack_attachment_text",
...
func encodeMattJamMeetingID(meeting string) string {
...
func (p *Plugin) askMeetingType(user *model.User, channel *model.Channel, rootID string) error {
	...
	apiURL := *p.API.GetConfig().ServiceSettings.SiteURL + "/plugins/mattjam/api/v1/meetings"
	...
	actions = append(actions, &model.PostAction{
		Name: p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "mattjam.ask.meeting_name_random_words",
	...
	actions = append(actions, &model.PostAction{
		Name: p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "mattjam.ask.personal_meeting",
	...
	if channel.Type == model.CHANNEL_OPEN || channel.Type == model.CHANNEL_PRIVATE {
		actions = append(actions, &model.PostAction{
			Name: p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
				DefaultMessage: &i18n.Message{
					ID:    "mattjam.ask.channel_meeting",
	...
	sa := model.SlackAttachment{
		Title: p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "mattjam.ask.title",
				Other: "MattJam Meeting Start",
			},
		}),
		Text: p.b.LocalizeWithConfig(l, &i18n.LocalizeConfig{
			DefaultMessage: &i18n.Message{
				ID:    "mattjam.ask.select_meeting_type",
...
func (p *Plugin) getUserConfig(userID string) (*UserConfig, error) {
	...
	if data == nil {
		return &UserConfig{
			Embedded:     p.getConfiguration().MattJamEmbedded,
			NamingScheme: p.getConfiguration().MattJamNamingScheme,
		}, nil
	}

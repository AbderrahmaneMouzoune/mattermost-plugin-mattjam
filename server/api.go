...
func (p *Plugin) ServeHTTP(c *plugin.Context, w http.ResponseWriter, r *http.Request) {
	...
	case "/api/v1/config":
		p.handleConfig(w, r)
	case "/mattjam_meet_external_api.js":
...
func (p *Plugin) handleExternalAPIjs(w http.ResponseWriter, r *http.Request) {
	if p.getConfiguration().MattJamCompatibilityMode {
...
func (p *Plugin) proxyExternalAPIjs(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := http.Get(p.getConfiguration().GetMattJamURL() + "/external_api.js")
	if err != nil {
		mlog.Error("Error getting the external_api.js file from your MattJam instance, please verify your MattJamURL setting", mlog.Err(err))
		http.Error(w, "Internal error", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		mlog.Error("Error getting reading the content", mlog.String("url", p.getConfiguration().GetMattJamURL()+"/external_api.js"), mlog.Err(err))
		http.Error(w, "Internal error", http.StatusInternalServerError)
		return
	}
...
func (p *Plugin) handleStartMeeting(w http.ResponseWriter, r *http.Request) {
	...
	if userConfig.NamingScheme == mattjamNameSchemeAsk && action.PostId == "" {

...
type Props = {
    ...
    jwt: string | null,
    actions: {
        openMattJamMeeting: (post: Post | null, jwt: string | null) => void
    }
}
...
export default class Conference extends React.PureComponent<Props, State> {
    ...
    preventMessages = (event: MessageEvent) => {
        ...
        if (event.origin !== meetingURL.origin && event.data) {
            ...
            if (data.postis && data.scope.indexOf('mattjam_meet_external_api_') === 0) {
    ...
    initMattJam = (post: Post) => {
        ...
        const options = {
            ...
            jwt: this.props.jwt,
            noSSL,
            parentNode: document.querySelector('#mattjamMeet'),
            onload: () => {
                this.setState({loading: false});
                this.resizeIframe();
            }
        };
        this.api = new (window as any).MattJamMeetExternalApi(domain, options);
    ...
    componentDidMount() {
        ...
        window.requestAnimationFrame(() => {
            if (this.props.post) {
                this.initMattJam(this.props.post);
            }
        });
    }
    ...
    close = () => {
        ...
        setTimeout(() => {
            this.props.actions.openMattJamMeeting(null, null);
            this.setState({
                minimized: true,
                loading: true,
                position: POSITION_BOTTOM,
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <div
                    id='mattjamMeet'
                    style={style.mattjamMeetContainer}
                />
